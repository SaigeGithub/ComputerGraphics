...
/*void main(){
    vec2 uvnew;
    uvnew.x=uv.x *3-float(uv.x *3);
    uvnew.y=uv.y *3-float(uv.y*3);
    vec3 sampleOrig =texture(tex,uvnew).rgb;
    if(uvnew.x>0.333)
    color =vec3(sampleOrig.r*2,sampleOrig.g*0.3,sampleOrig.b*0.3);
    else if(uvnew.x>0.666&& uvnew.x<1)
    color =vec3(sampleOrig.r*0.3, sampleOrig.g *2,sampleOrig.b*0.3);
    else
    color =vec3(sampleOrig.r*0.3, sampleOrig.g *0.3,sampleOrig.b*2);
    //float offset uvnew.x*2
    vec3 origsample=texture(tex,uv).rgb;
    color =vec3(origsample.r+uv.x,
                 origsample.g+uv.y,
                 origsample.b+(uv.x+uv.y)/2);
}*/
